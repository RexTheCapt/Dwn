...
namespace SubjectNerd.PsdImporter
{
    ...
    */
    public class EditorCoroutineRunner
    {
        ...
        /// <summary>
        ///     Start a coroutine and display a progress UI. only one EditorCoroutine can display a UI at once. equivilent of
        ///     calling StartCoroutine on a mono behaviour
        /// </summary>
        ...
        {
            if (uiCoroutineState != null && uiCoroutineState.showUI) uiCoroutineState.Label = label;
        }
        ...
        {
            if (uiCoroutineState != null && uiCoroutineState.showUI) uiCoroutineState.PercentComplete = percent;
        }
        ...
        private static void Runner()
        {
            ...
            for (var i = 0; i < coroutineStates.Count; i++) TickState(coroutineStates[i]);

            // if a coroutine was finished whilst we were ticking, clear it out now
            for (var i = 0; i < finishedThisUpdate.Count; i++)
            {
            ...
            if (coroutineStates.Count == 0) EditorApplication.update -= Runner;
        }
        ...
        private static void TickState(EditorCoroutineState state)
        {
            if (state.IsValid)
            {
                ...
                if (state.showUI && uiCoroutineState == state) uiCoroutineState.UpdateUI();
            }
            else
            {
            ...
            if (coroutine != null)
            {
                ...
                if (current != null)
                {
                    ...
                    else if (currentType.IsSubclassOf(typeof(CustomYieldInstruction)))
                    {
                        ...
                        if (yieldInstruction.keepWaiting) isWaiting = true;
                    }
                    else if (currentType == typeof(EditorCoroutine))
                    {
                        ...
                        if (!editorCoroutine.HasFinished) isWaiting = true;
                    }
                    ...
                    if (update)
                    {
                        ...
                        if (current != null)
                        {
                            ...
                            if (currentType == typeof(WaitForSeconds))
                            {
                                ...
                                var m_Seconds = typeof(WaitForSeconds).GetField("m_Seconds",
                                    BindingFlags.NonPublic | BindingFlags.Instance);
                                if (m_Seconds != null) timer = (float) m_Seconds.GetValue(wait);
                            }
                            else if (currentType == typeof(EditorStatusUpdate))
                            {
                                ...
                                if (updateInfo.HasLabelUpdate) Label = updateInfo.Label;
                                if (updateInfo.HasPercentUpdate) PercentComplete = updateInfo.PercentComplete;
                            }
                        }
                    }
                    else
